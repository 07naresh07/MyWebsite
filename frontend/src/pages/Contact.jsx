...
function useSmartSuggestions(message, contact) {
  return useMemo(() => {
    const suggestions = [];
    if (message.length < 20) suggestions.push({ type: "length", text: "Consider adding more details to your message" });
...
export default function EnhancedContact() {
  ...
  useEffect(() => {
    ...
    const hasPositive = positiveWords.some((w) => messageInput.toLowerCase().includes(w));
    const hasNegative = negativeWords.some((w) => messageInput.toLowerCase().includes(w));
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          >
            ...
            <div className="space-y-6">
              ...
              {/* Email Card - Clickable to open Gmail */}
              <motion.div
              ...
              {/* Phone Card - Clickable to open phone dialer */}
              <motion.div
                className={`p-6 rounded-2xl ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white"} border shadow-lg hover:shadow-xl transition-all duration-300 group cursor-pointer`}
                whileHover={{ y: -3, scale: 1.02 }}
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }}>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-xl border ${darkMode ? "bg-gray-700 border-gray-600 focus:border-blue-500" : "border-gray-300 focus:border-blue-500"} focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all ${contactInput && contactHint ? "border-red-300" : ""}`}
                      aria-describedby="contact-hint"
                      required
                    />
                    <p id="contact-hint" className={`mt-1 text-xs ${contactInput && contactHint ? "text-red-600" : "text-gray-500"}`}>
                      {contactInput ? contactHint || "Looks good." : "You can enter an email address or phone number."}
                    </p>
                  </motion.div>
                </div>

...
export default function EducationAdd() {
  ...
  // ✅ Relax validation so edits aren't blocked by level being empty
  useEffect(() => {
    const isValid = form.school.trim() && form.startYM; // level not required
  ...
  useEffect(() => {
    ...
    const e = current ? null : toDate(form.endYM);
      ...
      try {
        ...
        if (editing) {
          ...
          if (!rec) {
            ...
            const sy = rec.startYear ? String(rec.startYear).padStart(4, "0") : "";
            const sm = rec.startMonth ? String(rec.startMonth).padStart(2, "0") : "";
            const ey = rec.endYear != null ? String(rec.endYear).padStart(4, "0") : "";
            const em = rec.endMonth ? String(rec.endMonth).padStart(2, "0") : "";

            const startYM =
              rec.startYM && /^\d{4}-\d{2}$/.test(rec.startYM)
                ? rec.startYM
                : sy && sm
                ? `${sy}-${sm}`
                : sy
            ...
            const endYM =
              rec.endYear == null
                ? rec.endYM && /^\d{4}-\d{2}$/.test(rec.endYM)
                  ? rec.endYM
                  : ""
                : rec.endYM && /^\d{4}-\d{2}$/.test(rec.endYM)
                ? rec.endYM
            ...
            if (rec.thesis || rec.details) setShowAdvanced(true);
          }
        }
      } catch (e) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (!current && (eY && !eM)) {
        throw new Error(
          "Please complete End Date (YYYY-MM), or mark as Currently studying."
        );
      }
      ...
      const payload = {
        ...
        level: form.level, // optional now
        degree: form.degree.trim(),
        field: form.field.trim(),
        startYear: sY,
        startMonth: sM,
        startYM: `${String(sY)}-${String(sM).padStart(2, "0")}`,
        endYear: current ? null : eY ?? null,
        endMonth: current ? null : eM ?? null,
        endYM: current
          ? null
          : eY
          ? `${String(eY)}-${String(eM).padStart(2, "0")}`
          : null,

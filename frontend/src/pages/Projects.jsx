...
export default function Projects() {
  ...
  const goEdit = (id) => {
    if (!owner) return;
    nav(`/projects/edit/${encodeURIComponent(id)}`);
  };
...
function EnhancedEmptyState({ hasFilters, clearFilters, owner, onAdd, darkMode }) {
  return (
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className={`${darkMode ? "text-gray-400" : "text-gray-600"}`}>
            {hasFilters ? "Try adjusting your search criteria or filters." : "Start building your portfolio by adding your first project."}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          {/* Add button is owner-only now */}
          {owner && (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              onClick={onAdd}
              className="px-6 py-3 rounded-xl font-medium shadow-lg hover:shadow-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-700 hover:to-purple-700"
...
function EnhancedProjectsGrid({ projects, owner, busyId, onEdit, onDelete, onAdd, openModal, viewMode, darkMode }) {
  return (
    <div className="projects-grid">
      <AnimatePresence mode="popLayout">
        {projects.map((p, index) => {
          ...
          return (
            ...
            >
              {/* FIX: list view no longer forces a two-column flex with an empty left rail */}
              <div className={`h-full rounded-2xl shadow-lg border overflow-hidden relative ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} ${viewMode === "list" ? "" : "flex flex-col"}`}>
                ...
                {/* Owner controls (Edit/Delete) stay owner-only */}
                {owner && (
                ...
                <div className={`p-6 ${viewMode === "list" ? "" : "flex-1 flex flex-col"}`}>
                  ...
                  {/* Description (rich in list view, clamped in others) */}
                  <div>
                    {viewMode === "list" ? (
                      <div
                    ...
                    {/* Meta (only in list view to enrich details) */}
                    {viewMode === "list" && (

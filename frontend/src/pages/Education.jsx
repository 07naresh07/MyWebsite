...
function getYM(obj, prefix) {
  const yRaw = obj?.[`${prefix}Year`];
  const y = Number(yRaw);
  let m = obj?.[`${prefix}Month`];
  if (m != null) m = Number(m);
  if ((m == null || Number.isNaN(m)) && obj?.[`${prefix}YM`]) {
    const [yy, mm] = String(obj[`${prefix}YM`]).split("-");
    if (!Number.isNaN(Number(yy))) {
      if (obj[`${prefix}Year`] == null) obj[`${prefix}Year`] = Number(yy);
    }
...
function normalizeLevelText(s = "") {
  ...
  if (/ph\.?\s?d|doctor(al)?/.test(t)) return "PhD";
  if (/m\.?\s?phil/.test(t)) return "MPhil";
  if (/master|m\.?\s?s|m\.?\s?sc|mtech|m\.?\s?tech|m\.?\s?eng|msc/.test(t)) return "Master";
  if (/bachelor|b\.?\s?e|btech|b\.?\s?tech|b\.?\s?sc|b\.?\s?a|ba|be|beng|bsc/.test(t)) return "Bachelor";
  if (/diploma|certificate/.test(t)) return "Diploma/Certificate";
  if (/high\s*school/.test(t)) return "High School";
  if (/secondary/.test(t)) return "Secondary School";
  if (/primary/.test(t)) return "Primary School";
  return "";
}
function displayLevelName(it) {
  return normalizeLevelText(it?.level) || normalizeLevelText(it?.degree) || "";
}
function detectLevelRank(it) {
  const canonical = (displayLevelName(it) || it.level || "").toLowerCase();
...
function sortEdu(rows) {
  ...
  copy.sort((a, b) => {
    ...
    const aname = (a.school || "").toLowerCase();
    const bname = (b.school || "").toLowerCase();
    ...
    <div className="relative w-full overflow-hidden" ref={outerRef} aria-label={text} title={text}>
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .edu-marquee { animation: none !important; }
        }
      `}</style>
      <div
      ...
      <div className="container mx-auto py-10 px-4 pb-28">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className={`${darkMode ? "text-gray-400" : "text-gray-500"} mb-6`}>Try a different search term or adjust filters.</p>
            <button
              onClick={() => { setSearchTerm(""); setFilterType("all"); setSortBy("date"); }}
              className={`${darkMode ? "bg-gray-800 text-gray-100 hover:bg-gray-700 border-gray-700" : "bg-gray-200 text-gray-800 hover:bg-gray-300 border-gray-300"} px-4 py-2 rounded-lg border transition-colors`}
            >
              Clear Filters
            </button>
          </div>
...
function filterEducation(items, searchTerm, filterType, sortBy) {
  ...
  if (sortBy === "school") filtered.sort((a, b) => (a.school || "").localeCompare(b.school || ""));
